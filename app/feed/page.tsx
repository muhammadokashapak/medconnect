            ...
            <div className="space-y-6">
              {cases.map((c) => (
                <div key={c.id} onClick={() => router.push(`/case/${c.id}`)} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md cursor-pointer transition relative">
                  ...
                  <button onClick={(e) => toggleSave(e, c.id)} className="absolute top-6 right-6 text-gray-400 hover:text-yellow-500 transition z-10">
                    <svg className={`w-7 h-7 ${savedCaseIds.has(c.id) ? 'text-yellow-500' : ''}`} fill={savedCaseIds.has(c.id) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>
                  </button>
