        ...
        <div className="flex justify-between h-14 items-center">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative">
              <button onClick={() => {setIsMenuOpen(!isMenuOpen); setIsProfileOpen(false);}} className="w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-700 transition shadow-sm">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M5 3a2 2 0 100 4 2 2 0 000-4zm0 6a2 2 0 100 4 2 2 0 000-4zm0 6a2 2 0 100 4 2 2 0 000-4zM10 3a2 2 0 100 4 2 2 0 000-4zm0 6a2 2 0 100 4 2 2 0 000-4zm0 6a2 2 0 100 4 2 2 0 000-4zM15 3a2 2 0 100 4 2 2 0 000-4zm0 6a2 2 0 100 4 2 2 0 000-4zm0 6a2 2 0 100 4 2 2 0 000-4z"></path></svg>
              </button>
              {isMenuOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-gray-100 p-4 z-50">
                  <h3 className="text-xl font-bold mb-4">Menu</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <MenuCard icon="🏥" title="Hospital ERP" href="/erp" />
                    <MenuCard icon="🤖" title="AI Assistant" href="/ai" />
                    <MenuCard icon="👥" title="Patients Queue" href="/queue" />
                    <MenuCard icon="📅" title="Appointments" href="/appointments" />
                    <MenuCard icon="🔬" title="Laboratory" href="/erp/lab" />
                    <MenuCard icon="💊" title="Pharmacy" href="/erp/pharmacy" />
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Messages */}
            <Link href="/messages">
              <button className={`w-10 h-10 rounded-full flex items-center justify-center transition shadow-sm ${pathname.startsWith("/messages") ? "bg-blue-100 text-blue-600" : "bg-gray-100 hover:bg-gray-200 text-gray-700"}`}>
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7a8.841 8.841 0 01-4.083-.98L2 17l1.338-3.123C2.493 12.767 2 11.434 2 10c0-3.866 3.582-7 8-7s8 3.134 8 7zM7 9H5v2h2V9zm8 0h-2v2h2V9zM9 9h2v2H9V9z" clipRule="evenodd"></path></svg>
              </button>
            </Link>

            {/* Notifications */}
            <Link href="/notifications">
              <button className={`w-10 h-10 rounded-full flex items-center justify-center transition relative shadow-sm ${pathname === "/notifications" ? "bg-blue-100 text-blue-600" : "bg-gray-100 hover:bg-gray-200 text-gray-700"}`}>
            ...
            <div className="relative ml-2">
              <button onClick={() => {setIsProfileOpen(!isProfileOpen); setIsMenuOpen(false);}} className="w-10 h-10 rounded-full overflow-hidden border border-gray-300 hover:border-gray-400 transition shadow-sm">
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-xl border border-gray-100 py-2 z-50">
                  <Link href="/profile" className="block px-4 py-2 hover:bg-gray-50 flex items-center">
                    <span className="text-xl mr-3">👤</span> <span className="font-medium text-gray-900">Your Profile</span>
                  </Link>
                  <Link href="/saved-cases" className="block px-4 py-2 hover:bg-gray-50 flex items-center">
                    <span className="text-xl mr-3">🔖</span> <span className="font-medium text-gray-900">Saved Cases</span>
                  </Link>
                  ...
                  <button onClick={handleLogout} className="w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center text-red-600">
                    <span className="text-xl mr-3">🚪</span> <span className="font-medium">Logout</span>
                  </button>
                </div>
              )}
            </div>
